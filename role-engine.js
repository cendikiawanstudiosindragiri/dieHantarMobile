...
const RoleEngine = {
    ...
    renderUserDashboard(data) {
        const container = document.getElementById("page-home");
        if (!container) return;
        // Panggil kembali fungsi render service cards dari services.js
        if (window.renderHomePageServices) {
            window.renderHomePageServices(container);
        } else {
            container.innerHTML = `<p class="text-center text-gray-500">Gagal memuat menu layanan.</p>`;
        }
    },
    ...
    renderDevDashboard(data) {
        ...
        container.innerHTML = `
            <div class="p-8 text-center space-y-4">
                 <h2 class="text-lg font-black italic text-zinc-800">DEV-MODE</h2>
                 <p class="text-xs text-zinc-500">Data JSON dari /api/data:</p>
                 <pre class="bg-zinc-100 p-4 rounded-xl text-left text-[10px] overflow-auto">${JSON.stringify(data, null, 2)}</pre>
            </div>`;
    },
    ...
    injectDevSwitcher(data) {
        const container = document.getElementById("profile-menu-container");
        if (data.user.username === 'dev' && !document.getElementById("quantum-switcher") && container) {
            const switcherHtml = `
